...
static int engine_destroy(struct xdma_dev *xdev, struct xdma_engine *engine)
{
	...
	/*
	 * Stop the DMA engine and wait for it to quiesce BEFORE releasing
	 * descriptor buffers. Masking the interrupt alone leaves the engine
	 * potentially still fetching/writing descriptors via DMA, which can
	 * corrupt freed memory after dma_free_coherent() below.
	 */
	...
	/* Poll STATUS.BUSY clear with a bounded timeout (~100 ms) so we
	 * do not free descriptor memory while the engine is still active.
	 */
	{
		int busy_iter;

		for (busy_iter = 0; busy_iter < 1000; busy_iter++) {
			u32 st = read_register(&engine->regs->status);

			if (!(st & XDMA_STAT_BUSY))
				break;
			udelay(100);
		}
		if (busy_iter == 1000)
			pr_warn("engine %s still BUSY after stop, proceeding with teardown\n",
				engine->name);
	}
